...
|---|---|
| Shielded activation with useful uncertainty | `self_shielding` + `uncertainty` is supported: the MF=33 collapse weights each row's flux by the same per-group shield factors the depletion fold applies (`CovarianceLibrary::collapse_weighted`), with nominal cross sections checked bitwise against the shielded depletion convention. Current shielding is limited to the documented treatment; uncertainty omits flux, composition and other named sources. [Specification](SPEC.md) |
