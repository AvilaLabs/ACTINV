...
| **P5** API & spec | JSON problem spec (materials by wt-%/atom fraction, spectrum with group structure, schedule, options); `actinv run spec.json`; PyO3 `actinv.run(spec)` returning inventories, activity, heat split, ledger, certificate; pathway analysis (per-nuclide production chains ranked by contribution). | spec round-trip = CLI = Python at 0.0; pathway sums equal totals to 1e-12; planted-failure ledger control | P4 | 2–3 |
| **P6** v0.1 release | Wheels (maturin) and crate metadata; GitHub Actions running the control suite on a data subset; CHANGELOG, versioning, docs pass; reproducibility check on a second machine. Making the repository public is the principal's act. | CI green on a clean runner; certificate matches across two machines | P5 | 1–2 |
...
| **P10** Data completeness | R-matrix-limited (LRF=7) reconstruction; unresolved range with LSSF=0 (infinite-dilution averages from parameters); Doppler at arbitrary temperature; TENDL p/d/α activation; `actinv-data` as a standalone library generator. | LRF=7 vs NJOY on a FENDL W-186; URR averages vs NJOY UNRESR on a sample; p/d/α one-group values vs FISPACT-II reference runs | P4 | 4–5 |
