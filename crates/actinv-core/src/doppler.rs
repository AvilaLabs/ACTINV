...
#[inline]
fn poly_f_inf(c: &[f64; 5]) -> f64 { c[0] * SQPI / 2.0 + c[2] * SQPI / 4.0 + c[4] * 3.0 * SQPI / 8.0 }
...
pub fn broaden(e: &[f64], sig: &[f64], t_k: f64, awr: f64, eout: &[f64]) -> Vec<f64> {
    let n = e.len();
    assert!(n >= 2 && sig.len() == n, "grid and cross section must have the same length >= 2");
    let kt = KB * t_k / awr;
    ...
    for k in 0..n - 1 {
        let de = e[k + 1] - e[k];
        let slope = if de > 0.0 { (sig[k + 1] - sig[k]) / de } else { 0.0 };
        a[k] = sig[k] - slope * e[k];
        b[k] = slope * kt;
    }
    ...
    for (oi, &eo) in eout.iter().enumerate() {
        ...
        for &sign in &[1.0f64, -1.0] {
            ...
            let lo_x = yy - WINDOW;
            let hi_x = yy + WINDOW;
            let k0 = match x.binary_search_by(|p| p.partial_cmp(&lo_x).unwrap()) { Ok(i) => i, Err(i) => i.saturating_sub(1) };
            let k1 = match x.binary_search_by(|p| p.partial_cmp(&hi_x).unwrap()) { Ok(i) => i, Err(i) => i }.min(n - 1);
            for k in k0..k1 {
                let (t_lo, t_hi) = (x[k] - yy, x[k + 1] - yy);
                let (ak, bk) = (a[k], b[k]);
                let c = [ak * yy * yy + bk * yy.powi(4), 2.0 * ak * yy + 4.0 * bk * yy.powi(3), ak + 6.0 * bk * yy * yy, 4.0 * bk * yy, bk];
