#!/usr/bin/env python3
"""Generate crates/actinv-data/data/mt_products.json — the residual (dZ, dA) for each ENDF reaction MT, parsed from openmc.data's reaction
...
out = {"note": "Residual nuclide offset (dZ, dA) per reaction MT: residual = target + neutron - emitted particles. "
       ...
       "table": tab}
target = os.path.join(ROOT, "crates", "actinv-data", "data", "mt_products.json")
os.makedirs(os.path.dirname(target), exist_ok=True)
with open(target, "w") as stream:
    json.dump(out, stream, indent=1)
    stream.write("\n")
print(f"crates/actinv-data/data/mt_products.json: {len(tab)} reactions")
