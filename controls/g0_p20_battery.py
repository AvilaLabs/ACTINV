#!/usr/bin/env python3
"""P20 G0 producer: frozen identity battery at the opening commit.
...
`uncertainty` section is absent.

Additionally records the frozen Core case — the same example plus the real
full-corpus covariance sidecar — as the pre-P20 (P11-shape) uncertainty output
reference.
"""
...
RESULT = ROOT / "results/g0_p20_identity_baseline.json"
...
CORE_RESULT = ROOT / "target/p20-core/result.json"
COV_NPZ = ROOT / "target/p11-full-v2-repro.cov.npz"
THREADS = (
    "OMP_NUM_THREADS",
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="actinv-p20-g0-", dir=ROOT / "target"
    ) as directory:
        work = Path(directory)
        cache = work / "cache"
        ...
        # Core case: same example plus the real covariance sidecar (P11-shape
        # output reference).  Runs only when the pinned sidecar exists.
        core = None
        if COV_NPZ.is_file() and CORE_SPEC.is_file() and CORE_RESULT.is_file():
            core = {
                "spec_sha256": sha256(CORE_SPEC),
                "result_sha256": sha256(CORE_RESULT),
                "covariance_sha256": sha256(COV_NPZ),
            }
    ...
    evidence = {
        "schema": "actinv-p20-g0-baseline-1",
        "inputs": {
