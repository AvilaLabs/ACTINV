#!/usr/bin/env python3
"""P60 shared case machinery — banded measurement-design spec on the P58
fixture (its covariance carries a genuine cross-MT correlated block)
plus result extraction helpers used by every P60 gate.
"""
...
def spec(fxmap: dict, design=True, channels=("cross_section_mf33",
    ...
    if design:
        uncertainty["design"] = {}
    return {
        "spec": "actinv-spec-1", "title": "p60 case", "projectile": "neutron",
...
def design_block(result: dict, response: str, step=-1) -> dict:
    return result["steps"][step]["uncertainty"]["responses"][response]["design"]
