...
def vector_closure_failures(row_meta, sigma, n_groups: int) -> list[str]:
    ...
    for label, members, total_index in checks:
        ...
        for group in range(n_groups):
            ...
            # Governed bound: emitted <= total within binary rounding, or a
            # physically weightless excess below the P25 floor_reconciled
            # bound of 1e-15 barn absolute (builder FLOOR_EXCESS_ABS_B).
            if emitted > total * (1.0 + 1e-12) and emitted - total > 1e-15:
                failures.append(
