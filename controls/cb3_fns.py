#!/usr/bin/env python3
"""CB3: identical-data FNS arm - ACTINV on TENDL-2017 vs published FISPACT-II/TENDL-2017.

Removes the CB1/CB2 library confound: both arms use the TENDL-2017 evaluated
neutron library (FISPACT via its collapsed tendl17 distribution, ACTINV via an
ENDF-6 artifact built from the IAEA TENDL-2017 mirror). Residual confound:
decay data - FISPACT used its condensed tendl17_decay12 while this arm keeps
ENDF/B-VIII.0 primary + JEFF-3.3 fallback (different decay evaluations,
second-order on FNS heat metrics). The 34 FISPACT-reachable nuclides absent
from TENDL-2017 itself are an inherent library coverage boundary.
"""
...
LIBRARY = Path(
    os.environ.get("ACTINV_LIBRARY", DATA / "tendl-2017/build/neutron.n.p10.npz")
).resolve()
DECAY_PRIMARY = Path(
    os.environ.get(
...
EXPECTED = {
    "library": "218156708ae685db2a50cd7a2c74ca02049b68f266c38a29020b71095717447d",
    "library_index": "1f1124ab299c95aa0e0fb7caac52c47cb63ca56df688a7bdd9ab2dfecbc62a06",
...
def run_experiment(material: str, experiment: str, fingerprint: str) -> dict[str, object]:
    ...
    for measurement_index, cooling_index in matched:
        row = {
            ...
            "actinv_tendl2017_uW_g": float(calculated[cooling_index]),
        }
    ...
    metrics = {"actinv_tendl2017_uW_g": product_metrics(pairs, "actinv_tendl2017_uW_g")}
...
def main() -> None:
    ...
    for index, (material, experiment) in enumerate(experiments, 1):
        ...
        if index == 1 or index % 10 == 0 or index == len(experiments):
            print(
                ...
                f"points={record['metrics']['actinv_tendl2017_uW_g'].get('points', 0):2d} "
    ...
    output = {
        "schema": "actinv-cb3-fns-tendl2017-1",
        "access": {
            "ACTINV/TENDL-2017": "executed",
        ...
        "summary": {
            "actinv_tendl2017": aggregate(records, "actinv_tendl2017_uW_g"),
    ...
    checks = {
        ...
        "all_experiments_have_positive_measurement_pairs": all(
            record["metrics"]["actinv_tendl2017_uW_g"].get("points", 0) > 0
            for record in records
        ),
