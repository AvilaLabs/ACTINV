...
FNS_DIR = sys.argv[1] if len(sys.argv) > 1 else "fns"; CERT_NAME = sys.argv[2] if len(sys.argv) > 2 else "fns_certificate.json"
LIB_NPZ = os.environ.get("ACTINV_LIBRARY", os.path.expanduser("~/nuclear-data/eaf-2010/actinv_eaf2010_709g.npz"))
def sha(p): h = hashlib.sha256(); h.update(open(p, "rb").read()); return h.hexdigest()
inputs = {"library_npz": LIB_NPZ, "library_index": LIB_NPZ.replace(".npz", "_index.json"), "library_source_manifest": os.path.join(os.path.dirname(LIB_NPZ), "MANIFEST_zips.sha256"),
          "decay_endfb80": os.path.expanduser("~/nuclear-data/endfb-viii.0-decay/bulk/endf-b-viii-0_decay.dat"),
...
cert = {"program": "ACTINV", "instrument": "FNS decay-heat harness", "results_dir": FNS_DIR, "library": os.path.basename(LIB_NPZ), "inputs": {k: {"path": p, "sha256": sha(p) if os.path.exists(p) else None} for k, p in inputs.items()}, "experiments": {}}
for f in sorted(glob.glob(os.path.join(RES, FNS_DIR, "*.json"))):
...
json.dump(cert, open(os.path.join(RES, CERT_NAME), "w"), indent=1); print("certificate:", len(cert["inputs"]), "inputs,", len(cert["experiments"]), "experiments, body sha", cert["certificate_body_sha256"][:16])
