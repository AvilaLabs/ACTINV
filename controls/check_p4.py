...
v = {"gates": {}}
idx = load(os.path.join(LIB, "actinv_tendl2023_709g_index.json"))
if idx is None: v["gates"]["G1"] = "UNSCORED"
    ...
    v["gates"]["G1"] = ("PASS" if nerr == 0 and n >= 2848 else "FAIL") + f" (targets {n}, errors {nerr}, targets with ledger entries {led}, INCOMPLETE-URR {urr}, unsupported formalism/range {unsup}, rows {idx['n_rows']}, build {idx['build_seconds']/3600:.1f} h)"
...
cert = load(os.path.join(RES, "fns_tendl_certificate.json"))
